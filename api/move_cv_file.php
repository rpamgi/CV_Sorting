...
$source_dir = getRpaConfigValue($conn, 'N8NCVPATH', "C:\\Users\\administrator.MEGHNAGROUP\\.n8n-files\\CVs");
$dest_dir = getRpaConfigValue($conn, 'N8NCVDEST', "C:\\Users\\administrator.MEGHNAGROUP\\.n8n-files\\Prcoessed CV's");

// Ensure directories exist
if (!is_dir($source_dir)) {
    echo json_encode(["status" => "error", "message" => "Source directory not found: $source_dir", "path_hint" => "Update CV_SOURCE_DIR in RPA Config."]);
    exit;
}
if (!is_dir($dest_dir)) {
...
if (!empty($filename)) {
    ...
    if (file_exists($source_path)) {
        ...
        echo json_encode(["status" => "error", "message" => "File '$filename' does not exist in source directory."]);
    }
}
else {
